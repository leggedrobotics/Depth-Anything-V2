        ...
        for i, batch in enumerate(dataloader):
            ...
            for j in range(len(batch['paths'])):
                ...
                if args.grayscale:
                    ...
                    depth_viz = (cmap(depth_viz)[:, :, :3] * 255)[:, :, ::-1].astype(np.uint8)
            ...
            print(f"Processed {processed}/{len(dataset)} images, "
                  ...
                  f"Average per image: {batch_time/len(batch['paths']):.3f}s")
    
    elapsed = time.time() - start_time
    print(f"Processing complete! Total time: {elapsed:.3f}s, Average time per image: {total_time/len(dataset):.3f}s")
